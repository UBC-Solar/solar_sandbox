...
//Renode's STM Can Peripheral with slight state machine modification to work with STM32 HAL Code
...
{
    public class STM32F103_CAN : IDoubleWordPeripheral, ICAN, INumberedGPIOOutput
    {
        public STM32F103_CAN(STM32F103_CAN master = null)
        {
            // Register master CAN
            this.master = master;
        ...
        public void WriteDoubleWord(long address, uint value)  // cpu do per
        {
            ...
            switch((registerOffset)address)
            {
                ...
                case registerOffset.CAN_MCR:
                    ...
                    if (registers.CAN_MCR.SleepRequest == true && registers.CAN_MCR.InitRequest == false)
                    {
                    ...
                    else if (registers.CAN_MCR.SleepRequest == false && registers.CAN_MCR.InitRequest == true)
                    {
                        registers.CAN_MSR.InitAck = true;
                        registers.CAN_MSR.SleepAck = false;
                    }
                    else if (registers.CAN_MCR.SleepRequest == true && registers.CAN_MCR.InitRequest == true)
                    {
                        registers.CAN_MSR.InitAck = true;
                        registers.CAN_MSR.SleepAck = true;
                    }
        ...
        private readonly STM32F103_CAN master;
