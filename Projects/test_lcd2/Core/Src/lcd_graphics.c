...
static void glcd_pixel(uint8_t x, uint8_t y, uint8_t colour);
static void glcd_clear_bounding_box(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2);
static void glcd_refresh();
...
static uint8_t glcd_flipped = 0;

#ifdef ST7565_DIRTY_PAGES
static uint8_t glcd_dirty_pages;
#endif
 ...
 */
static void glcd_clear_bounding_box(unsigned char x1, unsigned char y1, unsigned char x2, unsigned char y2) {
...
static void glcd_refresh() {
    for (int y = 0; y < 8; y++) {
        ...
        LCD_write_command(GLCD_CMD_SET_PAGE | y);
        ...
        if (glcd_flipped) {
#endif
            LCD_write_command(GLCD_CMD_COLUMN_LOWER | 4);
        } else {
            LCD_write_command(GLCD_CMD_COLUMN_LOWER);
        }
        LCD_write_command(GLCD_CMD_COLUMN_UPPER);

        for (int x = 0; x < 128; x++) {
            LCD_write_data(glcd_buffer[y * 128 + x]);
        }
    }
...
static void draw_rectangle(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint8_t color)
{
    for(uint8_t x = x1; x <= x2; x++) {
        glcd_pixel(x, y1, color);
        glcd_pixel(x, y2, color);
    }
    for(uint8_t y = y1; y <= y2; y++) {
        glcd_pixel(x1, y, color);
        glcd_pixel(x2, y, color);
    }
}
...
void LCD_display_speed(uint32_t speed, int units)
{
    ...
    glcd_clear_bounding_box(SPEED_X - SPEED_SPACING, SPEED_Y, old_bb_speed.x2 + SPEED_SPACING, BOTTOM_RIGHT_Y);
    glcd_clear_bounding_box(old_bb_speed.x2 - SPEED_SPACING, SPEED_Y, old_bb_speed_units.x2 + SPEED_UNITS_SPACING, SPEED_Y + 11);
