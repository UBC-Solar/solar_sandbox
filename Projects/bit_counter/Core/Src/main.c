...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  //Generate 1000 mock CAN_msg_t
	  if(count < 1){
		  CAN_msg_t* test_msg = generateMockCAN(0x602, 3, 0);
		  bitCount += calculate_CAN_message_bits(test_msg);
		  free(test_msg);
		  count++;
	  }
...
/* USER CODE BEGIN 4 */
// function to calculate the number of bits in a given CAN message
